...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    await run('DELETE FROM clients WHERE id = ? AND company_id = ?', [req.params.id, req.user.companyId]);
...
router.post('/:clientId/sites', requireAuth, async (req, res) => {
  try {
    ...
    const client = await get('SELECT id FROM clients WHERE id = ? AND company_id = ?', [req.params.clientId, req.user.companyId]);
    if (!client) {
      return res.status(404).json({ error: 'Client not found' });
    }
...
router.put('/:clientId/sites/:siteId', requireAuth, async (req, res) => {
  try {
    ...
    `, [req.params.siteId, req.params.clientId, req.user.companyId]);
