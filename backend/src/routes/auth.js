...
const { getUserFeatures } = require('../middleware/subscriptionAccess');
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({
      ...
      // Keep additional tokens for enhanced features
      refreshToken,
      sessionToken,
    });
  } catch (error) {
...
router.post('/register', rateLimitSensitive(), async (req, res) => {
  try {
    const {
      email, password, name, companyName, plan = 'inspection_only',
    } = req.body;
    ...
    if (!company) {
      const result = await run(
        `INSERT INTO companies (
          ...
          subscription_status,
          subscription_plan
        ) VALUES (?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, datetime('now', '+14 days'), 'trial', ?)`,
        [companyName.trim(), plan, plan],
      );
...
// Get user subscription and feature access
router.get('/subscription', authenticateToken, requireAuth, async (req, res) => {
  try {
    ...
    const availableFeatures = await getUserFeatures(req.user.company_id);
    ...
    if (company.subscription_status === 'trial' && company.trial_ends_at) {
      ...
      trialInfo = {
        ...
        daysRemaining,
        endDate: company.trial_ends_at,
        startDate: company.trial_starts_at,
      };
    }
    ...
    res.json({
      ...
      planDisplayName: company.subscription_plan === 'inspection_only' ? 'Inspection Pro' : 'Business Pro',
    });
